...
Themes are used to modify existing components to fit a specific visual style. By using Made. Design Tokens, developers can easily customize all of their components by changing a set of "themeable" tokens, eliminating the need to modify individual component styles.

**Instantly re-theme according to your brand**  
By bringing semantic colors with smart role based tokens to a global color palette, designers and developers don't have to second-guess what color to apply for a background. They don't have to choose between gray.01 or white. Instead, they'd apply a design decision with an example name of `made-color-background-default`. By using this approach, changing from light to dark mode is only a matter of switching the values this color has, instead of re-applying the dark color styles.

**Creating new themes is a matter of minutes, not hours**  
Once set up correctly, creating a new theme is much easier than before. You'd use an existing theme as a basis and apply new decisions, e.g. switching the source for `made-color-background-default` from white (in light mode) to black (in dark mode). Or changing your accent color from orange to the color your brand is using, e.g. teal. With all those smart aliases in place, you don't have to re-apply new styles. Instead, the applied styles stay the same, only their value changes.

## What can design tokens should we theme?
...
See our working example on <a href="https://fusion.mastercard.int/stash/projects/MADE/repos/made-theming-example/browse" target="_blank">BitBucket</a>. 
